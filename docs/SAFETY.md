...
- **背景 `/usage` 偶爾可能要先啟動互動式 Claude Code。** 實機曾連續得到無法辨識的回應，直到使用者在 Terminal 啟動一次 `claude` 後才恢復。登入、續期或本機初始化是合理推測，但根因未由官方契約或可重現實驗確認；因此 App 只提供複製 `claude` 的人工 workaround，不自動登入、不改憑證，也不把這個現象降級成可信讀數。
