...
| guard: GitHub Actions | workflow 固定 `macos-26`／Xcode 26.6、以完整 SHA 鎖定 checkout、read-only token、不保留 credentials、不用 secrets／`pull_request_target`，並直接呼叫 `./scripts/verify.sh` | CI 應驗證陌生 clone，不應取得 push 或外部帳號權限，也不應分叉成另一套測試流程 |
| guard: public tree | gitignored 本機 skills、`.handoff/`、`docs/HANDOFF_to_*.md`、`docs/PUBLIC_GITHUB_PLAN.md` 與本機 usage probe 不得仍受追蹤；退役的 `spike-d1.sh` 不得存在 | 公開 repository 只包含產品、測試與使用者需要的文件，不把已知私人協作路徑或 provider 研究工具一起發布；任意研究文件仍須 code review，guard 不假裝能判斷內容語意 |
...
2026-09-01，公開前 security scan 與歷史清理準備：完整檢視 97 個 tracked paths，未發現 token、私鑰、真實 email、本機使用者路徑、Apple Team ID、簽章 identity、raw account response 或 credential；兩項 low finding 已修正。GitHub Actions 的 checkout 從可移動 v6 tag 改為官方 v6.1.0 的完整 commit SHA，靜態 guard 同時要求唯一 invocation 與 exact pin；Claude parser 的 source／test 註解移除真實觀察日期與精確 reset 時間，只保留可公開的行為理由，shipping source 另拒絕重新加入「日期加精確觀察」形狀。公開樹固定忽略並直接拒絕追蹤 `.handoff/`、`docs/HANDOFF_to_*.md`、`docs/PUBLIC_GITHUB_PLAN.md` 與本機 usage probe，不再宣稱 guard 能判斷任意研究文件的內容語意。診斷紀錄除 200 筆與 3／5／7 天限制外，持久化 JSON 在 decode 前另限 64 KiB；超限的合法 JSON 紅燈測試先證明舊版仍會解碼，修正後邊界內資料維持相容、超出一 byte 立即清除。版本文件移除會隨下次 prepare 過期的具體現況數字，統一指向 `Version.xcconfig` 與只讀 plan。Focused 診斷 8 項、Claude decoder 28 項與 shell／Git guard 通過；完整 verifier 通過 176 項／22 suites、Debug／Release Xcode builds、隔離 AppKit diagnostics、provider transition shutdown、雙語 settings／popover render、production settings unchanged、packaging／release fixtures、全部靜態護欄及 `git diff --check`。沒有執行真實 provider、查詢帳號、讀取 credential、建立 DMG、tag、push 或遠端 release。

2026-08-31，隱私安全的詳細錯誤紀錄：實機曾連續把 Claude 查詢記成 `schemaChanged`，開啟一次互動式 Claude Code 後恢復；舊紀錄只留下時間、provider 與大分類，無法分辨 JSON 外層、result、session／weekly 行或 reset 解析哪一層失敗。Focused 測試先因 `DiagnosticLogEntry` 沒有詳細原因與複製文字而編譯紅燈。修正後新紀錄另保存 optional、封閉的 `DiagnosticErrorDetail`；已知 Claude／Codex 錯誤形狀縮減成固定 App 原因代碼，未知 associated string 一律記為 `unrecognizedResponse`，動態百分比、任意 marker、路徑與 provider 原文不進持久化 bytes 或複製文字。舊版沒有 detail 的 JSON 仍可解碼並明示當時未保存更多細節。設定頁每列可個別展開、顯示依目前 App 語言產生的完整說明，並以使用者觸發的 pasteboard 按鈕複製同一份安全診斷；展開／已複製狀態不持久化。Focused 診斷 7 項通過，實際 Debug App target 編譯通過；完整 `./scripts/verify.sh` 通過 175 項／22 suites、Debug／Release Xcode builds、隔離 AppKit diagnostics、provider transition shutdown、雙語 settings／popover render、production settings unchanged、packaging／release fixtures、全部靜態護欄及 `git diff --check`。新增的 expanded diagnostics English／Dark 460×424 render 通過，人工檢視 `dist/ui-settings.png` 確認長訊息與 Copy Message 按鈕沒有裁切。沒有執行真實 provider、查詢帳號、讀取 credential、建立 DMG、tag、push 或遠端 release。
...
2026-08-26，GitHub Actions workflow：committed tree 原先沒有 workflow，先作為紅燈；新增 `.github/workflows/verify.yml`，固定 `macos-26`、Xcode 26.6 與 checkout v6，權限只有 `contents: read`，checkout 不保留 credentials，沒有 secrets、`pull_request_target` 或 write scope，且唯一驗證步驟直接呼叫 `./scripts/verify.sh`。YAML 解析與靜態 CI guard 通過；完整 verifier 通過 127 項／19 suites、Debug／Release builds、全部隔離 diagnostics 與雙語 render、production settings unchanged、打包 fixture、所有靜態護欄及 `git diff --check`。當時尚未有 GitHub hosted run，因 repository 尚未建立或推送；沒有執行真實 provider。後續公開前加固已將 action 鎖定為完整 commit SHA。
