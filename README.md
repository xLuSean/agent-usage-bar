...
不儲存用量歷史、不畫趨勢圖、不做用量預測。用量部分只落地設定值與**每個供應商最近一次的讀數** —— 永遠一筆，每次覆蓋，沒有可以拿來畫趨勢的序列。設定另有一份隱私安全的錯誤紀錄，只保存時間、供應商與 App 定義的錯誤類型；預設保留 5 天，可改為 3／7 天或隨時清除。
...
設定的「診斷紀錄」可查看最近查詢失敗的時間與類型。它不保存原始回應、憑證、執行檔路徑或供應商提供的任意文字，並固定限制為最新 200 筆。
