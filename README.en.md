...
Agent Usage Bar is not intended to be a complete AI usage analytics platform. It answers one question: **how much of the current Claude or Codex subscription allowance has been used?** To do that, the app deliberately does not read or store `auth.json`, OAuth tokens, Keychain credentials, conversation transcripts, task JSONL, or local databases. It also does not provide multi-account switching, cross-device sync, or long-term usage history.

The installed and signed-in official CLIs perform the queries: Claude runs a fixed read-only `/usage` command, and Codex uses the read-only App Server method `account/rateLimits/read`. The app keeps only the parsed values needed by the UI and the latest snapshot. Sign-in, renewal, and credential storage remain the responsibility of the official tools.

This boundary is an intentional product tradeoff, not an unfinished feature list. The app gives up per-request token history, cost trends, and multi-account management, but does not need the conversation content and login access those features would require. When an upstream format or data freshness cannot be trusted, the app shows Unknown instead of reading a more sensitive source to keep the UI populated.
...
The app does not store usage history, draw trends, or forecast usage. Usage persistence contains settings and **only the latest reading for each provider**—one snapshot that is overwritten, never a sequence suitable for charting. Settings also include a privacy-safe error history containing time, provider, an app-defined error category, and a closed app-defined detail reason. Each entry can be expanded and its full safe explanation copied. Raw responses and arbitrary provider text are never stored. Logs are retained for five days by default, configurable to three or seven days, and can be cleared at any time.

The latest snapshot exists to prevent excessive queries. Previously, every app launch immediately sent another request; closing and reopening the app ten times meant ten requests. The app now reuses the last reading on startup and queries only after the polling interval has elapsed.
...
The default is **10 minutes** (about 144 queries per day), configurable to 1, 3, 5, 10, 30, or 60 minutes. The interface shows the approximate daily request count for every option.
...
Over JSONL, the app sends only initialization and `account/rateLimits/read`, and accepts `account/rateLimits/updated` notifications. It does not read `auth.json`, task JSONL, SQLite, or caches, and it implements no method for login, logout, purchases, email, or consuming reset credits.

App Server is one long-lived child process, not a new process for every refresh. Disabling Codex, changing its CLI path, or quitting the app terminates only the exact process created by this app. If normal termination fails, the app waits briefly and then force-stops only that same process. A safety query runs every ten minutes by default. Push notifications update only the fields they actually provide; they do not clear weekly usage, Credits, plan, or delay a complete query. If a read times out, the app discards the old connection and initializes a new one on the next attempt; a healthy connection stays alive. OpenAI has not published a minimum interval or fixed penalty window for this read-only query. Ten minutes is this app's conservative default, not an official limit.
